Array.prototype.weightedRandom = function (p) {
    ...
    if (!p || !this[0][p] || typeof this[0][p] != "number") {
...
const mainComp = new Vue({
    ...
    methods: {
        ...
        drawStuff: function (e) {
            if (this.drawOn) {
                ...
                dctx.fillStyle = '#000';
                dctx.fillRect(e.offsetX, e.offsetY, 2, 2)
            }
        },
        nextFile: function (s) {
            const self = this;
        ...
        brainCycle: function (noLearn) {
            ...
            if (self.rounds.thisImg >= 100 || self.rounds.total === 0) {
                //need to get a new image
                self.rounds.total++;
                // console.log('Grabbing new image! Old one was', self.img.filename || 'nothing');
                self.rounds.thisImg = 0;
                if (noLearn) {
                    return alert('Your scores are', self.score)
                }
            ...
            //okay, now we know we have an image. Can continue with "brain" stuff
    ...
    computed: {
        // allActive: function () {
        //     //get all active neurons for this "run";
        //     return this.brain.layers.map(ly => {
        //         return ly.filter(na => !!na.isOn).map((q, i) => i);
        //     })
        // }
    }
